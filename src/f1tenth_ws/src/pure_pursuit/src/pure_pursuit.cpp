...
void PurePursuit::transformandinterp_waypoint() {  // pass old waypoint here
    ...
    try {
    	...
    	return; // MUST EXIT HERE! Wait for the next loop.
    }
...
double PurePursuit::get_velocity(double steering_angle) {
    ...
    // SAFE CHECK: Ensure the Velocity vector isn't empty before checking index
    if (!waypoints.V.empty() && waypoints.velocity_index < waypoints.V.size()) {
